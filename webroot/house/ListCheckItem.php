...
<?php
	$SQLstring = "SELECT * FROM `House_Item` WHERE checkStatus='0'";
		...
		<table class='table table-bordered table-condensed cf' border='1'>
			...
			<tbody>
				...
				if($numRows > 0) {
					...
					$itemID = $Row['id'];
					do {
						$SQLstring2 = "SELECT * FROM `House_Checkout` WHERE itemid=$itemID";
						...
						echo "<td><a href='/house/itemCheck.php?id=$itemID'>$name</a></td>";
