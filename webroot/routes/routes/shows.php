...
function shows_getAllFutureShows(){
    ...
    // Get upcoming shows
    /*$query = "SELECT s.`id`, s.`name`, s.`description`, s.`societyId`, soc.`slug` AS 'societySlug', soc.`name` AS 'societyName', soc.`type` AS 'societyType', D.`lastShowDate`, D.`firstShowDate`, YEAR(D.`firstShowDate`) AS 'year'
            FROM `Show` s
    ...
    $query = "SELECT `id`, `slug`, `name`, `description`, `societyId`, `societySlug`, `societyName`, `societyType`, `lastShowDate`, `firstShowDate`, `year`, `academicYear`
    ...
    foreach ($upcomingShows as $key=>$show) {
        $upcomingShows[$key]['image'] = shows_getPreferredImage($show['id']);
    }
