...
function shows_getShowUrl(array $opts){
    ...
    if($year && $society){
        ...
        if($slug){
            return $app->urlFor('show', [ 'show' => $slug, 'year' => $year, 'society' => $society ]);
        }
    }
    ...
    if($id){
        ...
        if($no_resolve){
            return $app->urlFor("show_byId", [ 'show' => $id ]);
        }
        ...
        return $app->urlFor('show', [ 'show' => $show['slug'], 'year' => $year, 'society' => $show['societySlug'] ]);
    }

    return $app->urlFor('shows');
}
...
$app->get(
    '/show',
    function () use ($app) {
        $app->redirect($app->urlFor('shows'), 301);
    }
);
...
$app->get(
    ...
    function ($show) use ($app) {
        ...
        if(isset($s)){
            ...
            $app->redirect($app->urlFor('show', $params), 301);
        }else{
            $app->notFound();
        }
    }
)->name("show_byId")->setConditions(['show' => '[0-9]+']);
