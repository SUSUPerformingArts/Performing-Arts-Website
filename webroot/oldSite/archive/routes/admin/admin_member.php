...
$app->post(
    ...
    function () use ($app) {
        ...
        if(isset($urlRedirect)){
            ...
            $app->redirect($app->urlFor("member", [ "member" => $user["id"] ]));
        }
    }
)->name("member_edit-post");
...
$app->get(
    ...
    function () use ($app) {
        $id = $_SESSION["user"]["id"];
        $app->redirect($app->urlFor("member", [ "member" => $id ]));
    }
)->name("me");
...
$app->get(
    ...
    $authenticate('is_logged_in'),
    function () use ($app) {
        $app->redirect($app->urlFor("member_edit"), 301);
    }
)->name("me_edit");
...
)->via('POST', 'PUT')->name('member_create-post');
