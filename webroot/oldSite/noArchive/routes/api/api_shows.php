...
function api_shows_addUrls($show){
    ...
    $show['uri'] = rtrim($app->request->getUrl() . $app->urlFor("pa_api_show", [ "show" => $show['id'], "type" => null ]), ".");
...
function api_get_show_dates($db, $showId) {
    $prep = $db->prepare("SELECT showDate FROM `ShowEvent` WHERE showId=?");
    $prep->bind_param("i", $showId);
    $prep->execute();
...
$app->get(
    ...
    function() use ($app){
        ...
        $futureOnly = isset($get["future"]) ? boolval($get["future"]) : null;
        $stageSocOnly = isset($get["stagesoc"]) ? boolval($get["stagesoc"]) : null;
        ...
        $query = "SELECT * FROM `Show_WithExpandedInfo` s WHERE 1=1 ";

        if ($futureOnly) {
            $query .= " AND firstShowDate > NOW()";
        } 

        if ($stageSocOnly) {
            $query .= " AND stagesoc=1 ";
        }

        if($name && $society){
            $query .= " AND s.`name` LIKE ? AND s.`societyName` LIKE ? ";
            if($limit){
                $query .= " LIMIT ?";
        ...
        foreach ($searchResult as $key => $val) {
            $searchResult[$key]['dates'] = api_get_show_dates($db, $val['id']);
...
$app->get(
    ...
    function ($show, $type = 'json') use ($app) {
        ...
        $query = "SELECT s.`id`, s.`slug`, s.`name`, s.`description`, s.`venueId`, s.`venueName`, s.`ticketSource`, s.`ticketDetails`, s.`stagesoc`, s.`societyId`, s.`societySlug`, s.`societyName`,
