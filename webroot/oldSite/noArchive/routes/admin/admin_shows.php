    ...
    function () use ($app) {
        ...
        if($app->config('mode') == 'production'){
            ...
            $showDescription = "(<a href='" . $app->request->getUrl() . shows_getShowUrl([ "id" => $newId, "no_resolve" => true ]) ."'>Show info</a>)<br>" .
                                "By <a href='" . $app->request->getUrl() . $app->urlFor("pa_society", [ "society" => $data["soc"]["id"] ]) . "'>" . $data["soc"]["name"] . "</a><br>" . 
            ...
            // Arrays are assigned by value in phpteamup
            $calObjects = [];
            foreach ($data["show_dates"] as $date) {
        		...
        		$teampUpRes = json_decode($client->post("https://api." . substr($data["soc"]["teamup_edit_link"], 8) . "/events", [
        			'json' => [
                ...
                $teamUpCalId = $teampUpRes['event']['id'];

                // If stagesoc invloved then add to their teamup cal too
                if (boolval($data['show_stagesoc'])) {
                    $c2 = new GuzzleHttp\Client(['headers' => ['Teamup-Token' => 'dde6f40ea11a17acc9ce675b19c14c2f1633aae17f4164051ffc7a590f36e105']]);
                
                    $c2->post("https://api.teamup.com/ks2cbd5abd2e3a21c5/events", [
                        'json' => [
                            "subcalendar_id" => 1977701,
                            "start_dt" => $start,
                            "end_dt" => $end,
                            "all_day" => false,
                            "rrule" => "",
                ...
                $query = "INSERT INTO `ShowEvent` (showId, showDate, googleEventId, teamupEventId)
                            VALUES (?, FROM_UNIXTIME(?), ?, ?)";

                $prep = $db->prepare($query);
                $prep->bind_param("iiss", $newId, $ts, $calId, $teamUpcalId);
                $prep->execute();
                // Error Check
            }
        }else{
...
$app->delete(
    ...
    function ($show) use ($app) {
        ...
        $app->redirect($app->urlFor("pa_shows"));
    }
)->name("pa_show_delete-post");
